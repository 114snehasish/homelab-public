...
- **E15.0 (persist-RG + scoped grant) is a local apply as Owner and lands before #98** — CI cannot create a resource group or assign roles, the same limitation `infra/identity` already documents for itself. Per [ADR-0009](adr/0009-tiered-persistence-park-resume.md) §2 the persist RG, the backup storage account and its `restic` container are created **out-of-band by a committed `az` script and never managed by Terraform** — the same never-touch pattern as `do-not-delete`/`listeninfratfstatesa`, and a stronger guarantee than `prevent_destroy`, which is one commit deep. E15.0 therefore reduces to role assignments over pre-existing scopes. Note there is **no built-in `Disk Contributor` role**: letting CI manage disks in that RG needs a *custom* role definition (Owner/UAA to create), deliberately without `Microsoft.Compute/disks/delete`.
...
| R22 | The VM's own credential can destroy the backups it writes (`forget --prune` runs there on a timer) — distinct from **R13**, which covers password *loss* rather than credential *use* | Settled in [ADR-0009](adr/0009-tiered-persistence-park-resume.md) §6c. **Shipped:** blob versioning + soft delete ([#98](https://github.com/114snehasish/homelab/issues/98)), retention **30 days** — the window must outlast a park, which lasts weeks, so 7 days would expire unobserved. Those settings are no longer reconciled by any `plan` (§2), so [#102](https://github.com/114snehasish/homelab/issues/102)'s drill asserts them. **Target state:** a *custom* data-plane role for the VM granting blob read/write/add but not `blobs/delete` (no built-in expresses this), with `forget --prune` moved into a scheduled CI job holding its own identity ([#100](https://github.com/114snehasish/homelab/issues/100)). Version-level immutability rejected — it makes pruning impossible for the lock window |
