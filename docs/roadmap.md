...
- **E15.0 (persist-RG + scoped grant) is a local apply as Owner and lands before #98** — CI cannot create a resource group or assign roles, the same limitation `infra/identity` already documents for itself.
...
| R22 | The VM's own credential can destroy the backups it writes (`forget --prune` runs there on a timer) — distinct from **R13**, which covers password *loss* rather than credential *use* | Blob versioning + soft delete ([#98](https://github.com/114snehasish/homelab/issues/98)) with a retention window exceeding a realistic detection window; consider moving `forget --prune` off the VM into a scheduled CI job with its own identity ([#100](https://github.com/114snehasish/homelab/issues/100)) |
