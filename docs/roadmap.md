...
| Persistence | **Managed disk (own RG) + restic → Blob Hot** | Block storage is the only safe home for live SQLite/Postgres; restic gives encrypted, deduplicated, versioned logical backups independent of any disk. Azure Files rejected for databases (SMB locking/corruption). **Hot, not Cool** — Cool's 30-day minimum retention penalises `forget --prune`, which deletes pack files continuously; see [ADR-0009](adr/0009-tiered-persistence-park-resume.md) (`#97`). |
...
- **E15's disk migration + mount contract land before E03.3** — Caddy is the first thing to write real state to `/data`; moving a near-empty disk to the persist RG is trivial, moving a live one is surgery. **Deviation, recorded here:** `#39` shipped before `#98`/`#99` landed — the disk was still near-empty at the time (a fresh `homelab-data-disk`, nothing but Caddy's ACME state), so the risk this ordering guards against was judged small enough to accept rather than block on. `#98`'s runbook must run `docker compose down` in `apps/caddy` immediately before the snapshot, so the migration never happens under a live ACME writer — that step is not optional cleanup, it is what makes shipping `#39` first safe in hindsight. **In the event it was not needed** (2026-09-12): `#98` ran with the lab already parked, so there was no VM, no running Caddy and no writer to quiesce. The instruction stands for any future migration performed against a running node.
- **E15.0 (persist-RG + scoped grant) is a local apply as Owner and lands before #98** — CI cannot create a resource group or assign roles, the same limitation `infra/identity` already documents for itself. Per [ADR-0009](adr/0009-tiered-persistence-park-resume.md) §2 the persist RG is created **out-of-band by a committed `az` script (`scripts/bootstrap-persist-rg.sh`) and never managed by Terraform** — the same never-touch pattern as `do-not-delete`/`listeninfratfstatesa`, and a stronger guarantee than `prevent_destroy`, which is one commit deep. E15.0 therefore reduces to role assignments over pre-existing scopes. Note there is **no built-in `Disk Contributor` role**: letting CI manage disks in that RG needs a *custom* role definition (Owner/UAA to create), deliberately without `Microsoft.Compute/disks/delete`. **Shipped as `Homelab Persist Disk Writer`**, exactly `Microsoft.Compute/disks/read` + `/write` — `write` is load-bearing twice, since attaching a disk sets its `managedBy` and Azure has no `disks/join/action`. **Amended by #205 (2026-09-12): there is no separate backup storage account at all** — the `restic` container lands in the existing state account `listeninfratfstatesa` and is owned by #100, so the script creates only the RG. That account is in `centralindia` while the lab is in `southindia`, which puts a cross-region hop on every backup and restore.
...
| R12 | Disk migration to the persist RG corrupts/loses data | **Retired 2026-09-12 by [#98](https://github.com/114snehasish/homelab/issues/98).** Done while the lab was parked and the disk near-empty, so the disk was *unattached* and no snapshot of a live ACME writer was ever needed. Executed as an `az resource move` rather than the planned snapshot-swap — a control-plane metadata change that preserves the disk object, its bytes and its `uniqueId`, with none of the snapshot-swap's `create_option` ForceNew trap (see [ADR-0009](adr/0009-tiered-persistence-park-resume.md) §2's second amendment). A full pre-move snapshot was retained 7 days as the rollback artifact, backed by an independent offline copy of `/data/caddy` |
...
| R22 | The VM's own credential can destroy the backups it writes (`forget --prune` runs there on a timer) — distinct from **R13**, which covers password *loss* rather than credential *use* | Settled in [ADR-0009](adr/0009-tiered-persistence-park-resume.md) §6c. **Pending, not shipped — corrected by #205 (2026-09-12):** blob versioning and soft delete are **account-level** settings, not per-container, and the `restic` container now lives in the existing state account `listeninfratfstatesa`, whose live state is soft delete **7 days** and versioning **off**. Reaching §6c's retention **30 days** (the window must outlast a park, which lasts weeks, so 7 days would expire unobserved) therefore also changes behaviour for `tfstate` and `secret-files` — a change to a *Never touch* resource, so it is now [#100](https://github.com/114snehasish/homelab/issues/100)'s explicit decision rather than #98's side effect. Those settings are reconciled by no `plan` (§2), so [#102](https://github.com/114snehasish/homelab/issues/102)'s drill asserts them. **Target state:** a *custom* data-plane role for the VM granting blob read/write/add but not `blobs/delete` (no built-in expresses this), with `forget --prune` moved into a scheduled CI job holding its own identity ([#100](https://github.com/114snehasish/homelab/issues/100)). Version-level immutability rejected — it makes pruning impossible for the lock window |

## Capacity honesty & cut order

Full scope is **13 epics / 60 PRs ≈ 90–120 hours** — more than a typical solo evenings-and-weekends month (50–70 h). Core = E01–E03, E05–E09 (+ E12.1/.2) ≈ 42 PRs, still ambitious. **E15 adds ~10 PRs (~18–26 h) in weeks 1–2 and is exempt from the cut order** — persistence is the philosophy of the lab, and everything after it stands on it; if time is short, cut deeper into the flex epics instead.
...
- **Tier 0 — always-on control (~free)**: tfstate (external), Key Vault, DNS zone, and the `restic` container inside that same external state account (no separate backup storage account — amended by #205).
...
- **Tier 2 — cold logical**: **restic → Azure Blob (Hot tier)**, encrypted/deduplicated/versioned; nightly app-consistent backups (`sqlite3 .backup`, `pg_dump`) plus a final backup as the first act of every park. State remains inspectable and restorable even with zero compute. **Hot, not Cool** — corrected by [ADR-0009](adr/0009-tiered-persistence-park-resume.md) §6b: Cool's 30-day minimum retention makes `forget --prune` pay a prorated early-deletion penalty on nearly every pack it deletes, and the storage saving it buys is ~₹11/mo. The credential is a **user-assigned managed identity scoped to the container**, never an account key (§6a).
...
Children (one PR each): **E15.0** [#205](https://github.com/114snehasish/homelab/issues/205) `homelab-persist-rg` + scoped CI grant — a **local-apply bootstrap that blocks E15.2**, CI cannot create the RG or assign itself roles in it · **E15.1** [#97](https://github.com/114snehasish/homelab/issues/97) [ADR-0009](adr/0009-tiered-persistence-park-resume.md) tiered persistence + park/resume · **E15.2** [#98](https://github.com/114snehasish/homelab/issues/98) disk migration into the persist RG (**R12** — the one dangerous op, done while the disk is near-empty; the backup storage account it originally also scoped is gone, per #205). **Shipped as an `az resource move`, not the planned snapshot-swap**: `create_option` is ForceNew and read back from Azure, so a snapshot-created disk plans as a destroy-and-recreate that `prevent_destroy` refuses. The move preserves the disk object outright. Also ships `scripts/write-persist-marker.sh`, the ADR-0009 §3 marker writer that E15.3 and E15.8 reuse · **E15.3** [#99](https://github.com/114snehasish/homelab/issues/99) mount contract v2 (VM recreate) · **E15.4** [#100](https://github.com/114snehasish/homelab/issues/100) restic-to-blob service (repo password: GitHub secret interim → Key Vault after E05; **R13**) · **E15.5** [#101](https://github.com/114snehasish/homelab/issues/101) park/resume workflows · **E15.6** [#102](https://github.com/114snehasish/homelab/issues/102) full lifecycle drill + runbook · **E15.8** [#206](https://github.com/114snehasish/homelab/issues/206) DR drill: restore to a fresh VM from blob alone · **E15.9** [#207](https://github.com/114snehasish/homelab/issues/207) capacity: `/data` sizing, retention policy, online growth, restic cache · **E15.7** [#103](https://github.com/114snehasish/homelab/issues/103) local-path-provisioner StorageClass (lands with E09).
...
- Never touch: RG `do-not-delete`, storage account `listeninfratfstatesa` (containers `tfstate`, `secret-files`, and `restic` from #100 — never grant at account scope), SSH key `homelab-vm-ssh-key-2`, RG `homelab-persist-rg`.
