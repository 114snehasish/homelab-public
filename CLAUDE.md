...
- GitHub secrets are split: network/storage/compute workflows use `ARM_*` secret names; dns/cloudflare use `AZURE_*` (plus `DNS_ZONE_NAME`, `RESOURCE_GROUP_NAME`, `CLOUDFLARE_*`). Both sets must exist. `_terraform.yml` implements the split with an `if:`-gated step that overrides `ARM_CLIENT_ID`/`ARM_CLIENT_SECRET`/`ARM_SUBSCRIPTION_ID`/`ARM_TENANT_ID` from the `AZURE_*` secrets via `$GITHUB_ENV`, scoped to `infra/dns`/`infra/cloudflare` only — a static `env:` entry can't vary per caller. Known inconsistency — don't standardize without asking (dies in E02.4).
