...
- `deploy-network.yml`/`deploy-storage.yml`/`deploy-compute.yml` are thin wrappers around `.github/workflows/_terraform.yml` (a `workflow_call`-only reusable workflow taking `working_directory`/`apply`/`ssh_source_ip`), each run under a `tf-<working_directory>` concurrency group. It pins the Terraform CLI version from the repo-root `.terraform-version` file. `deploy-dns.yml`/`deploy-cloudflare.yml` don't use it yet — still inline their own steps (`AZURE_*` secret naming makes them a separate migration).
